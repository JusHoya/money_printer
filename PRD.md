...
- FR-0.1 Remove from the bot registry and delete: `btc_hourly` bot + `ml_btc_hourly` strategy, `sol_15m`/`doge_15m`/`xrp_15m`/`eth_15m`/`btc_15m` bots, `ml_btc_15m.py`, `crypto_strategy.py`, `longshot_fader_v2.py`, `cross_spread_arb.py`, `counter_trade.py` (after confirming its analyzer output is unused). Retain `latency_arb.py` unregistered with a header comment stating its mothball status and revival preconditions.
  - *Phase 0 resolution (2026-07-24):* `counter_trade.py` is RETAINED — the confirmation failed: `CounterTradeAnalyzer` is actively wired into the orchestrator's market loop as a log-only hedge analyzer (`run_dashboard.py` imports, instantiates `live=False`, and calls `should_counter()`/`reset_cycle()`). Deleting it requires orchestrator-loop surgery beyond Phase 0 scope. Re-evaluate during the Phase 1 weather-lifecycle work; all other FR-0.1 deletions completed as written.
