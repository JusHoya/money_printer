...
def make_strategy_with_mock_momentum(**kwargs):
    ...
    strategy.momentum.should_confirm_sell = MagicMock(
        return_value=(True, "MOCKED", 0.8)
    )
    return strategy
...
class TestNTickConfirmation(unittest.TestCase):
    ...
    def setUp(self):
        self.strategy = make_strategy_with_mock_momentum(
            trend_confirm_ticks=3, cooldown_seconds=300
        )
    ...
    def test_single_tick_above_bull_no_signal(self):
        ...
        self.assertEqual(
            len(signals), 0, "Should NOT signal on 1 tick above bull trigger"
        )
    ...
    def test_two_ticks_above_bull_no_signal(self):
        ...
        self.assertEqual(
            len(signals), 0, "Should NOT signal on 2 ticks above bull trigger"
        )
    ...
    def test_three_ticks_above_bull_fires(self):
        ...
        signals = self.strategy.analyze(make_market("TEST-001", 0.80))
        self.assertGreater(
            len(signals), 0, "Should fire BULL after 3 consecutive ticks above trigger"
        )
    ...
    def test_interrupted_ticks_reset_counter(self):
        ...
        signals = self.strategy.analyze(make_market("TEST-001", 0.80))
        self.assertEqual(
            len(signals), 0, "Counter should reset when price returns to dead zone"
        )
    ...
    def test_bear_requires_three_ticks(self):
        ...
        signals = self.strategy.analyze(
            make_market("TEST-001", 0.20, bid=0.20, ask=0.20)
        )
        self.assertGreater(
            len(signals), 0, "Should fire BEAR after 3 consecutive ticks below trigger"
        )
...
class TestPostTradeCooldown(unittest.TestCase):
    ...
    def test_cooldown_blocks_immediate_re_entry(self):
        ...
        self.strategy.analyze(
            make_market("TEST-001", 0.79, timestamp=t + timedelta(seconds=1))
        )
        signals = self.strategy.analyze(
            make_market("TEST-001", 0.80, timestamp=t + timedelta(seconds=2))
        )
        ...
        self.strategy.analyze(
            make_market("TEST-001", 0.79, timestamp=t2 + timedelta(seconds=1))
        )
        signals2 = self.strategy.analyze(
            make_market("TEST-001", 0.80, timestamp=t2 + timedelta(seconds=2))
        )
    ...
    def test_cooldown_expires_allows_re_entry(self):
        ...
        for i in range(5):
            self.strategy.analyze(
                make_market(
                    "TEST-001", 0.50, timestamp=t_after + timedelta(seconds=i - 10)
                )
            )
        ...
        self.strategy.analyze(
            make_market("TEST-001", 0.79, timestamp=t_after + timedelta(seconds=5))
        )
        signals2 = self.strategy.analyze(
            make_market("TEST-001", 0.80, timestamp=t_after + timedelta(seconds=10))
        )
        self.assertGreater(
            len(signals2), 0, "Should allow signal after cooldown expires"
        )
...
class TestWidenedThresholds(unittest.TestCase):
    ...
    def test_price_at_045_no_bear_signal(self):
        ...
        signals = self.strategy.analyze(
            make_market("TEST-001", 0.45, bid=0.45, ask=0.45)
        )
    ...
    def test_exact_boundary_no_signal(self):
        ...
        self.assertEqual(
            len(signals_bull), 0, "Exactly at bull trigger should not signal"
        )

        signals_bear = self.strategy.analyze(
            make_market("TEST-001", 0.40, bid=0.40, ask=0.40)
        )
        self.assertEqual(
            len(signals_bear), 0, "Exactly at bear trigger should not signal"
        )
