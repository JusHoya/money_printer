...
class Crypto15mTrendStrategy(Strategy):
    ...
    def __init__(
        ...
        self.trailing_stop_delta = (
            trailing_stop_delta  # e.g. 0.05 means trail 5c behind peak
        )
        self.confirmation_delay = confirmation_delay
    ...
    def analyze(self, market_data: MarketData) -> List[TradeSignal]:
        ...
        if (
            price_to_monitor > self.bull_trigger
            and self.last_price <= self.bull_trigger
        ):
        ...
        # 2. BEAR BREAKOUT (YES < Bear Trigger) -> Betting NO
            ...
            price_to_monitor < self.bear_trigger
            and self.last_price >= self.bear_trigger
        ):
...
class CryptoHourlyStrategy(Strategy):
    ...
    def _predict_future_price(
        ...
        if len(self.price_history) % 5 == 0:
            logger.info(
                f"[Hourly] Trend: Slope={slope:.4f}/sec | Current=${Y[-1]:.2f} -> Pred=${predicted_price:.2f}"
            )
    ...
    def analyze(self, market_data: MarketData) -> List[TradeSignal]:
        ...
        if predicted_price > (strike_val + self.confidence_margin):
            ...
            if market_data.ask < 0.85 and market_data.ask > 0:
                logger.info(
                    f"[Hourly] 🚀 BULL SIGNAL: Pred ${predicted_price:.2f} > Strike ${strike_val} (Spot ${current_spot:.2f}). Ask {market_data.ask:.2f}. BUY YES."
                )
                signals.append(
                    TradeSignal(
                        symbol=symbol,
        ...
        # Case B: We predict LOWER than Strike (Bearish for YES)
            ...
            if market_data.bid > 0.15 and market_data.bid < 1.0:
                logger.info(
                    f"[Hourly] 📉 BEAR SIGNAL: Pred ${predicted_price:.2f} < Strike ${strike_val}. Market Bid {market_data.bid:.2f}. SELL YES."
                )
                signals.append(
                    TradeSignal(
                        symbol=symbol,
...
class CryptoArbitrageStrategy(Strategy):
    ...
    def analyze(self, market_data: MarketData) -> List[TradeSignal]:
        ...
        if extra.get("source") != "live_coinbase":
            return []
        ...
        self.price_history.append((now, spot_price))
        if (
            len(self.price_history) > self.window_size + 5
        ):  # Need slightly more history for ROC
            self.price_history.pop(0)

        # Warm Start Logic: Fill buffer if we have at least one point
        if len(self.price_history) < self.window_size:
            logger.info(
                f"[Strategy] ⚡ Warm Starting Crypto Buffer ({len(self.price_history)}/{self.window_size})..."
            )
        ...
        if np.isnan(momentum) or np.isnan(volatility) or np.isnan(roc):
            logger.warning(
                f"[Strategy] ⚠️ NaN Features Detected. Momentum: {momentum}, Vol: {volatility}, ROC: {roc}"
            )
            return []

        # Feature Summary for Debug
        logger.info(
            f"[Strategy] [ML] Features -> Mom: {momentum:.4f}, Vol: {volatility:.4f}, ROC: {roc:.6f}"
        )
        ...
        if self.model:
            ...
            X = pd.DataFrame(
                [[momentum, volatility, roc]], columns=["Momentum", "Volatility", "ROC"]
            )
            self.model.predict(X)  # validate input shape
            probability = self.model.predict_proba(X)[0][
                1
            ]  # Probability of Class 1 (Up)

            logger.info(
                f"[Strategy] [ML] Prob(UP)={probability:.4f} | Target: >{self.threshold} OR <{1.0-self.threshold}"
            )
        else:
            # Fallback Heuristic
        ...
        _ = f"kxbtcd-{yy}{mmm}{dd}{hh}"

        _ = round(spot_price / 100) * 100
        ...
        if probability > self.threshold:
            ...
            logger.info(
                f"[Strategy] [ML] SIGNAL: UP ({probability:.2f}) @ {market_data.ask:.2f}"
            )
        ...
        # If Probability < (1 - Threshold) -> SELL YES (Bet DOWN)
            ...
            logger.info(
                f"[Strategy] [ML] SIGNAL: DOWN ({conf:.2f}) @ {market_data.bid:.2f}"
            )
...
class Crypto15mLateSniper(Strategy):
    ...
    def analyze(self, market_data: MarketData) -> Optional[list]:
        ...
        if cycle_key != self._current_cycle_key:
            ...
            self._traded_this_cycle = False
            logger.info(
                f"[LateSniper] New cycle: {cycle_key} | Threshold: {self.max_odds:.2f}"
            )
        ...
        if chosen_odds > self.max_odds and not force_trade:
            logger.debug(
                f"[LateSniper] Odds {chosen_odds:.2f} > threshold {self.max_odds:.2f}, waiting..."
            )
            return None

        if force_trade and chosen_odds > self.max_odds:
            logger.info(
                f"[LateSniper] FORCE TRADE at min {minute_in_cycle}: odds {chosen_odds:.2f} (threshold {self.max_odds:.2f})"
            )
    ...
    def _handle_position_close(self, position: dict):
        ...
        if won:
            ...
            logger.info(
                f"[LateSniper] WIN @ odds {odds:.2f} -> threshold relaxed to {self.max_odds:.2f}"
            )
        else:
            self.max_odds = max(self.ODDS_FLOOR, self.max_odds - self.TIGHTEN_STEP)
            logger.info(
                f"[LateSniper] LOSS @ odds {odds:.2f} -> threshold tightened to {self.max_odds:.2f}"
            )
        ...
        if "STOP_LOSS" in reason:
            ...
            logger.info(
                f"[LateSniper] Counter-trade ARMED: {self._counter_side} on {self._counter_symbol}"
            )
