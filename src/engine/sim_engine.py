...
class SimulationEngine(ExecutionEngine):
    ...
    def execute(self, signal: TradeSignal) -> bool:
        print(
            f"[{self.strategy.name()}] EXECUTE: {signal.side.upper()} {signal.quantity} {signal.symbol} @ {signal.limit_price}"
        )
        self.trades += 1
    ...
    def run(self, steps: int = 5, symbol: str = None):
        ...
        for i in range(steps):
            ...
            print(
                f"Data: {data.extra.get('source', 'unknown')} @ {data.price if data.price else 'N/A'}"
            )
...
if __name__ == "__main__":
    ...
    sim = SimulationEngine(
        strategy=WeatherArbitrageStrategyV2(), data_providers=[MockNWSProvider()]
    )
    sim.run()
